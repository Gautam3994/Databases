// to improe performance we use indexes but if one stage in middle of a pipeline doesnt make us of index (or transform the data) others below it wont as well. SO try to sort and match and filter at the top.
// 16MB data only be returned as a result in the query.
// and 100MB ram per pipleine
...
// showing the query isn't covered(ftech in winning plan)
db.movies.aggregate(
  [
    {
      $match: {
...
db.movies.aggregate([
  ...
  {
    $project: {
      _id: 0, //removing this will cause the engine to not to look to database for _id just use index
      title_size: { $size: { $split: ["$title", " "] } }
    }
  },
  {
...
// verifying that it is a covered query(no fetch in winning)
db.movies.aggregate(
  [
    {
      $match: {
...
db.movies.aggregate([
  ...
  },
  {
    $group: {
      _id: {
        $size: { $split: ["$title", " "] } //remove unneccessary project to improve performance
      },
      count: { $sum: 1 }
    }
  },
